...
//    private final String currency;
    ...
    public MoveTokensBetweenAccounts(String fromAccountName, String toAccountName, Long quantity) {
    ...
    public String call() throws FlowException {
        ...
        AccountInfo buyerAccountInfo = UtilitiesKt.getAccountService(this).accountInfo(fromAccountName).get(0).getState().getData();
        AccountInfo sellerAccountInfo = UtilitiesKt.getAccountService(this).accountInfo(toAccountName).get(0).getState().getData();
        ...
        EHKDType ehkdType = new EHKDType();
        Amount<TokenType> amount = new Amount(quantity, ehkdType);

        //Buyer Query for token balance.
        QueryCriteria queryCriteria = QueryUtilitiesKt.heldTokenAmountCriteria(ehkdType, buyerAccount).and(QueryUtilitiesKt.sumTokenCriteria());
        List<Object> sum = getServiceHub().getVaultService().queryBy(FungibleToken.class, queryCriteria).component5();
        if(sum.size() == 0)
            throw new FlowException(fromAccountName + " has 0 token balance. Please ask the Bank to issue some cash.");
        else {
            Long tokenBalance = (Long) sum.get(0);
            if(tokenBalance < quantity)
                throw new FlowException("Available token balance of " + fromAccountName+ " is less than the cost of the ticket. Please ask the Bank to issue some cash if you wish to buy the ticket ");
        }
